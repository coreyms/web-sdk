 ...
 *
 * Manticore Mayhem plays no Spine assets — characters use the bonerutter rig format —
...
const boom = (name: string): never => {
	throw new Error(
		`Spine runtime is excluded from the Manticore Mayhem bundle (vite.config.js alias -> src/spineStub.ts); "${name}" was called. Remove the alias if this game genuinely needs Spine.`,
	);
};
