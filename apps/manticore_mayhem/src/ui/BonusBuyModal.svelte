<script lang="ts">
	// Feature menu on black glass (the Angry Mantis anatomy, re-skinned). The menu's NAME is
	// FEATURE_MENU_NAME in game/constants.ts — placeholder BAZAAR until Corey picks the final one.
	// (Was "CHOW LINE" on Angry Mantis; the layout below is unchanged, only the heading is a constant.)
	// was the meal-ticket
	...
	import { CONFIRM_COST_MULTIPLIER, FEATURE_MENU_NAME } from '../game/constants';
...
<ModalShell {open} onclose={close} {master} {scale} {left} {top} dim="rgba(6,4,10,0.62)">
	<div class="center" style:gap="{compact ? 10 : 14}px">
		<div class="panel am-glass" class:compact onclick={(e) => e.stopPropagation()} role="presentation">
			<div class="head">
				<span class="title am-stencil">{FEATURE_MENU_NAME}</span>
