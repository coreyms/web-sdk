<script lang="ts">
	...
	const logoW = $derived(kind !== 'portrait' ? 420 : 300);
	const barW = $derived(kind !== 'portrait' ? 360 : 260);
...
<FadeContainer show={loadingType === 'start'}>
	<MainContainer>
		<Container x={master.width * 0.5} y={master.height * 0.42}>
			<Sprite key={kind !== 'portrait' ? 'logoLandscape' : 'logoWide'} anchor={0.5} width={logoW} height={logoW * (kind !== 'portrait' ? 223 / 431 : 157 / 900)} />
			{#if fontsReady}
				<GameText text="WIN UP TO 20,000×" preset="gold" size={kind !== 'portrait' ? 26 : 18} y={kind !== 'portrait' ? logoW * 0.36 : 60} />
			{/if}
			{#if !context.stateApp.loaded}
				<Container x={-barW / 2} y={kind !== 'portrait' ? logoW * 0.46 : 90}>
