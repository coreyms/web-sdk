<script lang="ts">
	...
	const MODES = [
		...
		{ id: 'feast', label: 'Mantis Feast', accent: '#b07a12', cost: soc('Not sold directly', 'Not offered directly'), costNum: 0, enter: soc('Land 5 Marky scatters, or win it inside a Mystery Spin.', 'Land 5 Marky scatters, or win it inside a Mystery spin.'), spins: `${config.freeSpins.feast} Free Spins.`, mech: soc(`Marty AND Marky feed: two opening bites, and both mantises strike. Every Mantis Feast session pays at least ${config.feastMinWin}× the bet.`, `Marty AND Marky feed: two opening bites, and both mantises strike. Every Mantis Feast session wins at least ${config.feastMinWin}× the play amount.`) },
